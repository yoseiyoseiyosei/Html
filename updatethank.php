...
if (!empty($_POST['action_text'])) {
    ...
    updateaction($text,$actionid);
    unset($_SESSION['actionid']);
}
if (!empty($_POST['do_text'])) {
    $text=$_POST['do_text'];
    $reportid=$_SESSION['reportid']; 

    if (!empty($_FILES['do_pic'])) {
        $pic=$_FILES['do_pic']['name'];
    }else{
        $tbl_name="report_tbl";
        $tbl_id="report_id";
        $reportInfo=tbl($tbl_name,$tbl_id,$reportid);
        $pic=$reportInfo['do_pic'];
        
    }
    if (!empty($_POST['do_movie'])) {
        $movie=$_POST['do_movie'];
    }else{
        $tbl_name="report_tbl";
        $tbl_id="report_id";
        $reportInfo=tbl($tbl_name,$tbl_id,$reportid);
        $movie=$reportInfo['do_movie'];
    }
    updatereport($text,$pic,$movie,$reportid);
    ...
    <div class="container">
                        <div class="row-fluid">
                            <div class="span contact-form centered">
                                <h3>変更できました。</h3>
                                <p><a href="index.php">HOMEへ戻る</a></p>
                            </div>
                        </div>
        </div>
</div>
